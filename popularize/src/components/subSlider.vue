<template>
  ...
  <div class="pop-content city-mode" id="drawerCity">
    ...
    <div class="container">
      ...
      <div id="wrapperCity" class="wrapper">
        <div class="list-group" id="listCitySub">
          <ul class="letter-index">
          </ul>
        </div>
      </div>
  ...
  <!-- 选择车型弹层 S -->
  <div class="pop-content car-type-mode" id="drawerType">
    <div class="title row">
      <span class="close" @click="hideDrawer">关闭</span>
      <span class="txt">选择车型</span>
    </div>
    <div class="container">
      <!-- 滚动区 S -->
      <div class="wrapper" id="wrapperType">
        <div id="listType">
          <ul class="car-style"></ul>
        </div>
      </div>
      <!-- 滚动区 E -->
    </div>
...
<style>
  #drawerPro, #drawerCity, #drawerType {
    display: none;
  }
...
<script>
  ...
  export default{
    ...
    methods: {
      hideDrawer: function () {
        $("#layer").fadeOut(400);
        $("#drawerPro,#drawerCity,#drawerType").animate({
          "left": "100%"
        }, 400, function () {
          $(this).hide();
